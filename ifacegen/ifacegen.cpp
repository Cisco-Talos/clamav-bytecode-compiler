...
private:
  ...
  std::vector<PATypeHolder> opaqueTypes;
  ...
  std::map<std::string, PATypeHolder> namedTypes;

  PATypeHolder ParseType(tok::kind token, bool &Ok)
  {
    std::map<std::string, PATypeHolder>::iterator namedType;
    if (token == tok::End) {
    ...
    case tok::Struct:
      ...
      if (namedType == namedTypes.end()) {
        // new struct type
        PATypeHolder Ty = OpaqueType::get(C);
        opaqueTypes.push_back(Ty);
        namedTypes.insert(std::pair<std::string, PATypeHolder>(CurString, Ty));
        return Ty;
      }
  ...
  tok::kind ParseTypeFully(const Type *&Ty, tok::kind token) {
    TypeFlags = 0;
    bool Ok = true;
    PATypeHolder hTy = ParseType(token, Ok);
    if (!Ok)
      return tok::Error;
    Ty = hTy.get();
  ...
  bool ParseStructDecl(PATypeHolder STy) {
    const Type *Ty;
  ...
  const Type* parseOptionalArraySize(tok::kind &token, const Type *Ty)
  {
...
void Parser::outputHeader(raw_ostream &Out, const std::string HeaderName)
{
  ...
  Out << " *  Copyright (C) 2009-2010 Sourcefire, Inc.\n"
    << " *  All rights reserved.\n *\n"
...
bool Parser::output(raw_ostream &Out, raw_ostream &OutImpl, raw_ostream &OutHooks)
{
  ...
  for (GlobalMapTy::iterator I=globals.begin(), E=globals.end();
       I != E;) {
    Out << "\t{\"" << I->getKey() << "\", GLOBAL" <<
      UppercaseString(I->getKey().substr(8).str()) << ", "
      << typeIDs[I->getValue()]
  ...
  for (FunctionListTy::iterator I=functions.begin(), E=functions.end();
    ...
    if (FTy == Api0Ty) {
      ...
      errs() << "Unknown prototype: " << *FTy <<
        " for function " << I->first << "\n"
        << "known prototypes: "
        << *Api0Ty << "\n"
        << *Api1Ty << "\n"
        << *Api2Ty << "\n"
...
int main(int argc, char *argv[])
{
  ...
  SourceMgr SrcMgr;
  sys::Path Path = GetExecutablePath(argv[0]);
  Path = Path.getDirname();
  Path.appendComponent("..");
  Path.appendComponent("lib");
  Path.appendComponent("clang");
  Path.appendComponent(CLANG_VERSION_STRING);
  Path.appendComponent("include");
  ...
  raw_ostream *OutImpl = new raw_fd_ostream(OutputImplHeader.c_str(), ErrorInfo);
  if (!ErrorInfo.empty()) {
    errs() << ErrorInfo << '\n';
    delete OutImpl;
    return 1;
  ...
  // Make sure that the Out file gets unlinked from the disk if we get a
  // SIGINT
  sys::RemoveFileOnSignal(sys::Path(OutputImplHeader));

  raw_ostream *OutHooks = new raw_fd_ostream(OutputHooksHeader.c_str(), ErrorInfo);
  if (!ErrorInfo.empty()) {
    errs() << ErrorInfo << '\n';
    delete OutHooks;
    return 1;
  ...
  // Make sure that the Out file gets unlinked from the disk if we get a
  // SIGINT
  sys::RemoveFileOnSignal(sys::Path(OutputHooksHeader));

  LLVMContext &C = getGlobalContext();
  SrcMgr.AddNewSourceBuffer(Buffer, SMLoc());
  Parser parser(SrcMgr, C);
  if (!parser.parse())
    return 1;
