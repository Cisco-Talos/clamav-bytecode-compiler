# Copyright (C) 2017-2021 Cisco Systems, Inc. and/or its affiliates. All rights reserved.

"""
Wrapper for unittest to provide ClamBC-Compiler specific test environment features.
"""
...
class TestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Useful paths for running the tests.
        ...
        # Paths to applications we can test with.
        cls.clambcc =     Path(os.getenv("CLAMBCC"))  if os.getenv("CLAMBCC") != None else None
        ...
        # A list of headers directory paths.
        cls.headers_paths = [cls.path_source / 'headers']

        #TODO: Uncomment the following when the clamav-derived headers are removed
        #      from <src>/headers and are instead sourced from a clamav installation.
        # clamav_derived_headers = os.getenv("HEADERS")
        # if clamav_derived_headers != None and clamav_derived_headers != '':
        #     cls.headers_paths.append(Path(clamav_derived_headers))

        # A string for the compiler, like "-I /path/to/source/headers -I /usr/local/lib/clambc-headers/0.104.0"
        cls.headers = ' '.join(['-I {}'.format(path) for path in cls.headers_paths])
        ...
        # cls.log.info(f"  clambcc:           {cls.clambcc}")
    ...
    def setUp(self):
        ...
        log_path = Path(self.path_build / 'test' / '{}{}'.format(self._testMethodName, self.log_suffix))
        try:
            log_path.unlink()
        except Exception:
