...
void ClamBCModule::describeType(llvm::raw_ostream &Out, const Type *Ty, Module *M)
{
...
void ClamBCModule::printModuleHeader(Module &M, unsigned startTID, unsigned maxLine)
{
    ...
    unsigned maxfunc     = 0;
    if (MinFunc) {
        minfunc = cast<ConstantInt>(MinFunc->getOperand(0)->getOperand(0))->getZExtValue();
    }
    if (MaxFunc) {
        maxfunc = cast<ConstantInt>(MaxFunc->getOperand(0)->getOperand(0))->getZExtValue();
    }
