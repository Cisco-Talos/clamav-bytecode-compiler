                           ...
                           raw_fd_ostream *fd, CompilerInstance &Clang)
{
    ...
    createStandardModulePasses(&Passes, optimize,
                               ...
                               false,
                               false,
                               optimize > 1 ? createFunctionInliningPass(threshold) : createAlwaysInlinerPass());
    if (optimize) {
                             ...
                             bool versionOnly, sys::Path &apiMapPath)
{
    ...
    if (!FrontendOpts.Inputs.empty()) {
        ...
        if (TmpRe2C.createTemporaryFileOnDisk(true, &ErrMsg)) {
            Clang.getDiagnostics().Report(clang::diag::err_drv_unable_to_make_temp) << ErrMsg;
            return 1;
        }
        ...
        if (ret) {
            Clang.getDiagnostics().Report(clang::diag::err_drv_command_failed) << "re2c" << ret;
            return 1;
        }
        Input = TmpRe2C.str();
    }
