VIRUSNAME_PREFIX("BC.PDF.Parser")
VIRUSNAMES("MalwareFound")
...
static force_inline int decode_js_text(uint32_t *probTable,
...
static force_inline int extract_obj(uint32_t *probTable, unsigned *entropySize, unsigned pos, unsigned jsnorm)
{
  char filters[1025];
  unsigned filtern;
  //debug("extractobj");
...
static int force_inline handle_pdfobj(uint32_t *probTable,
...
static force_inline int32_t find_xref(unsigned *flags)
{
  ...
  if (matches(Signatures.PDF_EOF)) {
    int32_t eofpos = match_location(Signatures.PDF_EOF, 0);
    if (matches(Signatures.PDF_EOF_startxref)) {
      int32_t xrefpos = match_location(Signatures.PDF_EOF_startxref, 0);
      if (xrefpos < eofpos)
        return xrefpos;
    }
  }
